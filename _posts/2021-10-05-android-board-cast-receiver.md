...
## <span style="color:#0f7b6c">3. 시스템 메시지</span>

- 안드로이드 단말기에서는 사건이 발생했을 경우 각 사건에 대해 정해진 메시지를 발생시킨다.
- 메시지가 발생되면 이에 반응하는 Broad Cast Receiver들이 동작한다.
- 실제로는 개발자가 각 사건에 대한 이름으로 Broad Cast Receiver를 등록해 놓으면 OS가 이를 찾아 동작시키는 방식이다.
- 안드로이드는 8.0 부터 사용할 수 있는 시스템 메시지 수가 줄었다.
...
안드로이드 8.0 부터는 manifest.xml의 intent filter에 기록된 이름으로 Broad Cast Receiver를 동작시킬 수 없다. 하지만 위 링크에 기록된 리시버의 경우 예외적으로 동작한다. 
즉, 어플리케이션이 종료상태더라도 사용할 수 있다.